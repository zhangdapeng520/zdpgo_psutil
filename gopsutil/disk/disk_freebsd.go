...
func PartitionsWithContext(ctx context.Context, all bool) ([]PartitionStat, error) {
	...
	for _, stat := range fs {
		...
		d := PartitionStat{
			Device:     common2.ByteToString(stat.Mntfromname[:]),
			Mountpoint: common2.ByteToString(stat.Mntonname[:]),
			Fstype:     common2.ByteToString(stat.Fstypename[:]),
			Opts:       opts,
		}
...
func IOCountersWithContext(ctx context.Context, names ...string) (map[string]IOCountersStat, error) {
	...
	for i := 0; i < count; i++ {
		...
		name := common2.IntToString(d.Device_name[:]) + un

		if len(names) > 0 && !common2.StringsHas(names, name) {
			continue
		}
...
func parsedevstat(buf []byte) (devstat, error) {
	...
	err := common2.Read(br, binary.LittleEndian, &ds)
	if err != nil {
		return ds, err
	}
...
func getFsType(stat unix.Statfs_t) string {
	return common2.ByteToString(stat.Fstypename[:])
}
