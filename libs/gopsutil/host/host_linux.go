...
func HostIDWithContext(ctx context.Context) (string, error) {
	sysProductUUID := common2.HostSys("class/dmi/id/product_uuid")
	machineID := common2.HostEtc("machine-id")
	procSysKernelRandomBootID := common2.HostProc("sys/kernel/random/boot_id")
	switch {
	// In order to read this file, needs to be supported by kernel/arch and run as root
	// so having fallback is important
	case common2.PathExists(sysProductUUID):
		lines, err := common2.ReadLines(sysProductUUID)
		if err == nil && len(lines) > 0 && lines[0] != "" {
			return strings.ToLower(lines[0]), nil
		}
		fallthrough
	// Fallback on GNU Linux systems with systemd, readable by everyone
	case common2.PathExists(machineID):
		lines, err := common2.ReadLines(machineID)
	...
	default:
		lines, err := common2.ReadLines(procSysKernelRandomBootID)
		if err == nil && len(lines) > 0 && lines[0] != "" {
			return strings.ToLower(lines[0]), nil
		}
	}
...
func UsersWithContext(ctx context.Context) ([]UserStat, error) {
	utmpfile := common2.HostVar("run/utmp")
	...
	for i := 0; i < count; i++ {
		...
		user := UserStat{
			User:     common2.IntToString(u.User[:]),
			Terminal: common2.IntToString(u.Line[:]),
			Host:     common2.IntToString(u.Host[:]),
...
func getlsbStruct() (*lsbStruct, error) {
	ret := &lsbStruct{}
	if common2.PathExists(common2.HostEtc("lsb-release")) {
		contents, err := common2.ReadLines(common2.HostEtc("lsb-release"))
		if err != nil {
	...
	} else if common2.PathExists("/usr/bin/lsb_release") {
...
func PlatformInformationWithContext(ctx context.Context) (platform string, family string, version string, err error) {
	...
	if common2.PathExists(common2.HostEtc("oracle-release")) {
		platform = "oracle"
		contents, err := common2.ReadLines(common2.HostEtc("oracle-release"))
		if err == nil {
			version = getRedhatishVersion(contents)
		}

	} else if common2.PathExists(common2.HostEtc("enterprise-release")) {
		platform = "oracle"
		contents, err := common2.ReadLines(common2.HostEtc("enterprise-release"))
		if err == nil {
			version = getRedhatishVersion(contents)
		}
	} else if common2.PathExists(common2.HostEtc("slackware-version")) {
		platform = "slackware"
		contents, err := common2.ReadLines(common2.HostEtc("slackware-version"))
		if err == nil {
			version = getSlackwareVersion(contents)
		}
	} else if common2.PathExists(common2.HostEtc("debian_version")) {
		if lsb.ID == "Ubuntu" {
			platform = "ubuntu"
			version = lsb.Release
			...
			if common2.PathExists("/usr/bin/raspi-config") {
				platform = "raspbian"
			} else {
				platform = "debian"
			}
			contents, err := common2.ReadLines(common2.HostEtc("debian_version"))
			if err == nil && len(contents) > 0 && contents[0] != "" {
				version = contents[0]
			}
		}
	} else if common2.PathExists(common2.HostEtc("redhat-release")) {
		contents, err := common2.ReadLines(common2.HostEtc("redhat-release"))
		if err == nil {
			version = getRedhatishVersion(contents)
			platform = getRedhatishPlatform(contents)
		}
	} else if common2.PathExists(common2.HostEtc("system-release")) {
		contents, err := common2.ReadLines(common2.HostEtc("system-release"))
		if err == nil {
			version = getRedhatishVersion(contents)
			platform = getRedhatishPlatform(contents)
		}
	} else if common2.PathExists(common2.HostEtc("gentoo-release")) {
		platform = "gentoo"
		contents, err := common2.ReadLines(common2.HostEtc("gentoo-release"))
		if err == nil {
			version = getRedhatishVersion(contents)
		}
	} else if common2.PathExists(common2.HostEtc("SuSE-release")) {
		contents, err := common2.ReadLines(common2.HostEtc("SuSE-release"))
		if err == nil {
			version = getSuseVersion(contents)
			platform = getSusePlatform(contents)
		}
		// TODO: slackware detecion
	} else if common2.PathExists(common2.HostEtc("arch-release")) {
		platform = "arch"
		version = lsb.Release
	} else if common2.PathExists(common2.HostEtc("alpine-release")) {
		platform = "alpine"
		contents, err := common2.ReadLines(common2.HostEtc("alpine-release"))
		if err == nil && len(contents) > 0 && contents[0] != "" {
			version = contents[0]
		}
	} else if common2.PathExists(common2.HostEtc("os-release")) {
...
func SensorsTemperaturesWithContext(ctx context.Context) ([]TemperatureStat, error) {
	...
	if files, err = filepath.Glob(common2.HostSys("/class/hwmon/hwmon*/temp*_input")); err != nil {
	...
	if len(files) == 0 {
		...
		if files, err = filepath.Glob(common2.HostSys("/class/hwmon/hwmon*/device/temp*_input")); err != nil {
			return temperatures, err
		}
	}
	...
	if len(files) == 0 { // handle distributions without hwmon, like raspbian #391, parse legacy thermal_zone files
		files, err = filepath.Glob(common2.HostSys("/class/thermal/thermal_zone*/"))
		if err != nil {
			return temperatures, err
		}
