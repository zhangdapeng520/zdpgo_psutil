...
func PartitionsWithContext(ctx context.Context, all bool) ([]PartitionStat, error) {
	...
	for _, stat := range fs {
		...
		d := PartitionStat{
			Device:     common2.IntToString(stat.F_mntfromname[:]),
			Mountpoint: common2.IntToString(stat.F_mntonname[:]),
			Fstype:     common2.IntToString(stat.F_fstypename[:]),
			Opts:       opts,
		}
...
func IOCountersWithContext(ctx context.Context, names ...string) (map[string]IOCountersStat, error) {
	...
	for i := 0; i < count; i++ {
		...
		name := common2.IntToString(d.Name[:])
...
func getFsType(stat unix.Statfs_t) string {
	return common2.IntToString(stat.F_fstypename[:])
}
