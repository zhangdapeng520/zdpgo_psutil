...
import (
	...
	net2 "github.com/zhangdapeng520/zdpgo_psutil/libs/gopsutil/net"
	"io"
	"os"
	"reflect"
	"strings"
...
var (
	...
	procQueryFullProcessImageNameW = common2.Modkernel32.NewProc("QueryFullProcessImageNameW")
	procGetPriorityClass           = common2.Modkernel32.NewProc("GetPriorityClass")
	procGetProcessIoCounters       = common2.Modkernel32.NewProc("GetProcessIoCounters")
	procGetNativeSystemInfo        = common2.Modkernel32.NewProc("GetNativeSystemInfo")
...
func (p *Process) ExeWithContext(ctx context.Context) (string, error) {
	...
	return common2.ConvertDOSPath(windows.UTF16ToString(buf[:])), nil
}
...
func (p *Process) StatusWithContext(ctx context.Context) ([]string, error) {
	return []string{""}, common2.ErrNotImplementedError
}

func (p *Process) ForegroundWithContext(ctx context.Context) (bool, error) {
	return false, common2.ErrNotImplementedError
}
...
func (p *Process) OpenFilesWithContext(ctx context.Context) ([]OpenFilesStat, error) {
	...
	process, err := windows.OpenProcess(common2.ProcessQueryInformation, false, uint32(p.Pid))
	if err != nil {
		return nil, err
	}
	...
	st := common2.CallWithExpandingBuffer(
		func() common2.NtStatus {
			return common2.NtQuerySystemInformation(
				common2.SystemExtendedHandleInformationClass,
				&buffer[0],
				uint32(len(buffer)),
				&size,
			)
	...
	handlesList := (*common2.SystemExtendedHandleInformation)(unsafe.Pointer(&buffer[0]))
	handles := make([]common2.SystemExtendedHandleTableEntryInformation, int(handlesList.NumberOfHandles))
...
func (p *Process) ConnectionsWithContext(ctx context.Context) ([]net2.ConnectionStat, error) {
	return net2.ConnectionsPidWithContext(ctx, "all", p.Pid)
}

func (p *Process) ConnectionsMaxWithContext(ctx context.Context, max int) ([]net2.ConnectionStat, error) {
	return nil, common2.ErrNotImplementedError
}
...
func is32BitProcess(h windows.Handle) bool {
	...
	case PROCESSOR_ARCHITECTURE_AMD64:
		...
		ret, _, _ := common2.ProcNtQueryInformationProcess.Call(
			uintptr(h),
			uintptr(common2.ProcessWow64Information),
